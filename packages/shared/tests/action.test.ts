...
describe("ActionSchema", () => {
  it("accepts a move at a cell", () => {
    const action = { type: "move", playerId: "home-winger", target: { x: 2, y: 3 } };
    expect(ActionSchema.safeParse(action).success).toBe(true);
  });

  it("accepts a dribble at a cell", () => {
    const action = { type: "dribble", playerId: "home-striker", target: { x: 4, y: 2 } };
    expect(ActionSchema.safeParse(action).success).toBe(true);
  });

  it("accepts a pass at a team-mate", () => {
    const action = { type: "pass", playerId: "home-striker", target: "home-winger" };
    expect(ActionSchema.safeParse(action).success).toBe(true);
  });

  it("accepts a tackle on the carrier", () => {
    const action = { type: "tackle", playerId: "away-defender", target: "home-striker" };
    expect(ActionSchema.safeParse(action).success).toBe(true);
  });

  it("accepts a shot, which has no target — v1 shots are not aimed", () => {
    expect(
      ActionSchema.safeParse({ type: "shoot", playerId: "home-striker", target: null }).success,
    ).toBe(true);
  });

  it("rejects a move whose target is a player rather than a cell", () => {
    const action = { type: "move", playerId: "home-winger", target: "home-striker" };
    expect(ActionSchema.safeParse(action).success).toBe(false);
  });

  it("rejects a pass whose target is a cell rather than a player", () => {
    const action = { type: "pass", playerId: "home-striker", target: { x: 1, y: 1 } };
    expect(ActionSchema.safeParse(action).success).toBe(false);
  });

  it("rejects an unknown action type", () => {
    const action = { type: "header", playerId: "home-striker", target: null };
    expect(ActionSchema.safeParse(action).success).toBe(false);
  });
