...
> - **v1.12 → v1.13:** a stored match now records **which edition of the rules it was played under** (§15). A seed and a command log reproduce a match only against the rules that produced them — and the failure that matters is silent: a log written before ADR 0021 still replays end to end, to a match that never happened. Matches saved before this exists keep loading and are marked as unplaceable rather than discarded. See ADR 0022.
...
- The action menu (each costs one of your 2 actions per turn; only the goalkeeper gets Launch):
  ...
  - **Pass** — send the ball to **any team-mate within your PAS range whose lane is clear**. The lane is what the ball flies over: draw the line between the two cells, and a cell is under it when the flight crosses that cell's square — clipping a corner does not count, so a diagonal crosses only the cells it actually passes through. The first body under the flight stops the ball, whichever side it is on, so a team-mate in the way receives it instead. An angled ball (two forward, one across) has **two** cells under it and is therefore easier to block than a diagonal of the same length — which is what keeps the angles honest. **Automatic** if no opponent is beside the lane; a **duel** (passer **PAS** vs interceptor **DEF**) if one is. Either way the pass is offered, and the odds are shown before you commit.
  - **Launch** — a **goalkeeper** carrying the ball may kick it long: the same lane and the same first-body-blocks rule as a Pass, but out to the format's **`launchRange`** (§13) rather than to the kicker's PAS, and only to a team-mate **beyond** its own passing range — so a Pass and a Launch are never offered for the same team-mate. An outlet to relieve a press or start a counter. A clear lane is **automatic**, exactly like a Pass; a lane with an opponent beside it is the same interception duel (**PAS** vs **DEF**) **plus `LAUNCH_INTERCEPT_BONUS` to the defence**, because the ball is in the air long enough to be read. Lose it and the interceptor collects, the same as a Pass. `launchRange` is roughly half the board, so a keeper on its line can always reach the opposition half when the lane is clear.
  - **A won dribble carries on** — whatever a dribble was aimed at, winning it takes the carrier **one cell further in the direction of travel** when that cell is on the board, empty and one it may stand on. Losing is unchanged. Composed with the rule below, beating the man in front puts the carrier past him _and_ on.
